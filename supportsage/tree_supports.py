...
# ─── Helper: print quality inspection ────────────────────────────

def _print_inspection_report(image_path: str) -> None:
    """Lazy-import printsight and run analysis on the photo."""
    try:
        from printsight.analyzer import analyze
    except ImportError:
        print(
            "  🔍 PrintSight not installed. Install with: pip install printsight\n"
        )
        return
    ...
    try:
        ...
        print(f"  ⚠️  Print inspection failed: {e}\n")
        return

    if result.get("error"):
        print(f"  ⚠️  Print inspection: {result['error']}\n")
        return

    print(f"  🔍 Print Quality Inspection:")
    print(f"     Score: {result.get('quality_score', '—'):.1f} / 100")
    print(f"     Grade: {result.get('grade', '—')}")

    for key, label in [("stringing", "Stringing"), ("layer_quality", "Layer Quality"), ("warping", "Warping")]:
        info = result.get(key)
        if info:
            score = info.get("score", "—")
            details = info.get("details", "")
            print(f"     • {label}: {score}" + (f" — {details}" if details else ""))

    img = result.get("image", {})
    if img:
        print(f"     📸 {img.get('width', '?')}×{img.get('height', '?')} px")
    print()
...
def add_tree_parser(subparsers):
    ...
    parser.add_argument(
        ...
        help="Path to a JPEG/PNG photo of the print for quality analysis (requires printsight)"
    )
    return parser
...
def tree_command(args):
    ...
    if args.inspect:
        _print_inspection_report(args.inspect)
